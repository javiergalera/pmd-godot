...
func configure(start_tile: Vector2i, map_tile_size: float, enemy_manager: Node) -> void:
	tile_size = map_tile_size
	tile_position = start_tile
	manager = enemy_manager
	position = tile_center_3d(tile_position)
...
func move_to_tile(target_tile: Vector2i, duration: float) -> void:
	...
	_move_tween.tween_property(self, "position", tile_center_3d(target_tile), duration)
