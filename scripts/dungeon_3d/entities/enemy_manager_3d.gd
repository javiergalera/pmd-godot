extends Node3D
## 3D enemy manager — spawning, turn coordination, combat with defeat blink.
...
@onready var player = get_node_or_null("../Player3D")
...
func reset_enemies(spawn_tiles: Array[Vector2i], tile_size: float, seed_value: int, player_tile: Vector2i) -> void:
...
func kill_enemy_at(tile: Vector2i, attacker, attacker_tile: Vector2i) -> bool:
	var target := get_enemy_at(tile)
	if target == null:
		return false
	await CombatResolver.resolve_attack(attacker, attacker_tile, target, target.tile_position, true)
	if target.has_method("play_defeat_blink"):
		await target.play_defeat_blink()
