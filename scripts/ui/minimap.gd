...
func _reveal(center: Vector2i) -> void:
	for dx in range(-vision_radius, vision_radius + 1):
		for dy in range(-vision_radius, vision_radius + 1):
			...
			if tile.x >= 0 and tile.x < FLOOR_MAX_X and tile.y >= 0 and tile.y < FLOOR_MAX_Y:
				explored[tile] = true
...
func _flood_reveal_room(start: Vector2i) -> void:
	...
	while stack.size() > 0:
		...
		if visited.has(tile):
			continue
		visited[tile] = true
		if tile.x < 0 or tile.x >= FLOOR_MAX_X or tile.y < 0 or tile.y >= FLOOR_MAX_Y:
			continue
		explored[tile] = true
		if _dungeon_map.get_tile_type(tile) == "room":
